...
5. **Define commands per component.** `test`, `build`, `lint`, `typecheck`, plus a top-level
   `deploy` (or `deploy_apple`/`deploy_google` for a mobile store release). These are what the build
   loop and `/qa`/`/ship` actually run — get them right.
