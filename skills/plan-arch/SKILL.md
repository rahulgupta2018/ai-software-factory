...
3. **Choose the shape.** Decide languages and components (e.g. `api`, `web`, `db`). For each
   ...
   React path; justify any deviation against the PRD. Sketch the **module map** (the packages within
   each component — e.g. `engine`, `coach`, `persistence`, `ui`) and the **dependency rule** (which
   layer may import which; the domain core depends on nothing) — you'll record these in step 10.
...
5. **Select craft skills.** List the skills the build loop composes in `skills[]`. For each, make
   ...
   product.
6. **Decide the security & release bindings.** Walk the applicable gates for *this* stack and fill
...
7. **Set guardrails + escalation.** From the PRD's risk surface: `budget.warn_tokens`,
...
8. **Write `.factory/stack.yaml`, then prune it.** The machine half only. **Never touch `PRD.md`'s
...
9. **Sync + validate.** Run `fac sync-context` (merges → `.factory/context.gen.yaml` +
...
10. **Diagram the architecture.** Produce, as Mermaid validated with `fac diagram check`, at least:
    ...
    ```
    Embed the fenced ```mermaid blocks in the record (step 11); optionally render with
    `fac diagram svg|html`.
11. **Record the decision as a run artifact.** Under an active run, write the architecture
    *document* — not just the stack values, but the **module map + dependency rule**, the
    **diagrams** from step 10, the **concurrency & error-handling boundaries** (what runs off the
...
12. **Hand off.** Point the user to `/plan-design` (if any component has a UI) and then the build
...
Output: .factory/stack.yaml —
        ...
        + run artifact 02-plan-arch.md — module map (api: routes/handlers/db; web: ui/state) +
          dependency rule + a Mermaid component diagram + a sequence diagram for the "log a repair"
          write path + concurrency/error boundaries + test strategy. inputs: PRD.md + 01-discover.md.
