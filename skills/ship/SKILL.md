...
2. **Enforce upstream gates.** Confirm `/review` is clean of security findings and `/qa` is clean
   ...
   route back — do not ship. The supply-chain gate counts here too: a fix-available dependency
   finding at or above `tech_bindings.supply_chain.block_severity` (`lib/sca-report.ts`) or a
   missing SBOM blocks the ship until fixed or explicitly overridden.
