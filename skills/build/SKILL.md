...
2. **For each component** in `tech_stack.components[]`:
   ...
   b. **Install deps into the chosen isolation + check prerequisites.** First resolve
      ...
      (language-native local env: Python `venv`, JS `node_modules`); a production-grade ship uses a
      `container` (`docker build` per service — each ships a Dockerfile — so deps bake into the image,
      never the host). Then run `commands.<name>.install` (deps + dev tools, pulled from the manifest)
      inside that isolation, before the first test, so `test`/`lint`/`typecheck` have what they need. A
      runtime or system prerequisite the component requires but the environment lacks
      (`tech_stack.runtimes` / `tech_stack.prerequisites` — e.g. the Postgres/Redis for integration
