...
5. **Log durable lessons** as decisions and record the retro. A retro reflects across **many** runs,
   so it uses the **reflection band** (`--seq 9`) — outside the 1–6 product pipeline, marking a
   cross-run reflection rather than a linear step. It records into the active run (add `--id` to
   target a specific one); the durable record is the decision log, the artifact is the write-up:
   ```bash
   ...
   fac run artifact --seq 9 --step retro --body-file retro.md
   ```
...
Output: retro.md —
        ...
        09-retro.md recorded; 1 durable decision logged.
