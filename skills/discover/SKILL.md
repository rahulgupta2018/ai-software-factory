...
6. **Draft the PRD.** Write/update the product's `PRD.md` — the human-owned half only:
   - Body: fill the template's sections (problem, users, journeys, goals, functional + data model,
     NFRs, integrations, compliance, features, metrics, constraints, risks, out-of-scope, open
     questions). Leave a section with a one-line "TBD — <why>" rather than deleting it.
