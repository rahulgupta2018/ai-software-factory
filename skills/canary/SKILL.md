...
5. **Record the verdict.** The canary watches the deploy, so it is a **branch artifact** with a
   sub-sequence seq under ship (the next free letter after the deploy tracks, e.g. `6d`), recording
   the deploy log it monitors as its input:
   ```bash
   fac run artifact --seq 6d --step canary --inputs .factory/runs/$RUN/06a-deploy.md --body-file canary.md
   ```
...
Output: HARD STOP at t+90s. canary.md — /search regressed to 500; escalation triggered
        per policy with disclaimer; recommend rollback. 06d-canary.md recorded.
...
- Run harness (`fac run`) — records the verdict as a sub-sequenced `06d-canary.md`.
