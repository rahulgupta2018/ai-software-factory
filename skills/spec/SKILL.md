...
6. **Write the spec as a run artifact.** A spec sharpens one slice, so it is a **branch artifact**
   with a sub-sequence seq — the next free letter under the step it follows (typically `/plan-arch`,
   step 2). Multiple slices take successive letters (`2b`, `2c`, …):
   ```bash
   fac run artifact --seq 2b --step spec --inputs PRD.md --body-file spec.md
   ```
   Pass `--inputs PRD.md` when a PRD exists; **omit `--inputs`** for a standalone slice — the request
   is chat intent, not a file, so there is nothing upstream to hash.
...
Output: run artifact 02b-spec.md. Handoff → Implementer + tdd (each criterion → a test).
```
