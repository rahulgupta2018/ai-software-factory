...
- **The release log is the artifact.** Merge, CI result, deploy output, and health verification are
  recorded as a sub-sequenced run artifact (`06a-deploy.md`) — an auditable trail of what shipped and that it's
  healthy.
...
- **Build provenance is verified before release (§Phase 7).** A release must ship only the artifact
  ...
  `lib/provenance-verify.ts`: signature valid, keyless, digest **pinned to and matching** the
  artifact being deployed (`/deploy` supplies that digest — the verifier fails closed without it),
  and the OIDC identity / issuer / builder / source match the expected values, with a Rekor
  transparency-log entry. A missing, unverified, unpinned, or mismatched attestation **blocks the release**.
...
4. **Provenance gate — HARD GATE.** Before deploying, verify the release artifact's build
   ...
   `lib/provenance-verify.ts` against `tech_bindings.provenance`. **Set `expectedDigest` to the
   digest of the artifact you are about to deploy** — it is not in the binding (it changes every
   build), and without it the verifier fails closed (`digest-unpinned`): a provenance check that
   isn't pinned to *this* artifact's digest only proves an attestation exists, not that it covers
   what's shipping. A missing, unverified, key-based, unpinned, or mismatched attestation **blocks
   the release** — do not deploy an artifact you cannot prove the origin of.
...
9. **Write the release log as a run artifact.** The deploy tail runs after `/ship` (step 6), so its
   artifacts are **branch artifacts** with a sub-sequence seq under ship — the umbrella release log,
   then the per-store mobile tracks:
   ```bash
   fac run artifact --seq 6a --step deploy --inputs .factory/runs/$RUN/06-ship.md --body-file deploy-log.md
   ```
...
gate fails closed. **Pass the declared store set** (from `tech_bindings.mobile_release`) to
`verifyMobileReleases(observations, expectedStores)` so a declared store that produced no build
fails closed rather than an empty batch silently passing.
...
- **Track 1 — Apple App Store (`deploy-apple`).** Build a signed `.ipa` (`flutter build ipa` with a
  ...
  ```bash
  fac run artifact --seq 6b --step deploy-apple --inputs <ipa-manifest> --body-file deploy-apple.md
  ```
- **Track 2 — Google Play (`deploy-google`).** Build a signed **`.aab`** (`flutter build appbundle
  ...
  artifact:
  ```bash
  fac run artifact --seq 6c --step deploy-google --inputs <aab-manifest> --body-file deploy-google.md
  ```
...
Output: run artifact 06a-deploy.md — merge sha, CI link, deploy output, health-check result.
        Release verified.
...
- Run harness (`fac run`) — records the release as `06a-deploy.md`.
