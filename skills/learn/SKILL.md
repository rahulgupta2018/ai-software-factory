...
4. **Write the updated rule set** back to project memory:
   ...
   ```
5. **Log a governing decision** when the lesson is a standing choice, and record the pass. `/learn`
   promotes **cross-task** lessons, so its artifact uses the **reflection band** (`--seq 9`) —
   outside the 1–6 product pipeline — recording into the active run (add `--id` to target one). The
   durable record is memory + the decision log; the artifact is the write-up:
   ```bash
   ...
   fac run artifact --seq 9 --step learn --body-file learn.md
   ```
...
- Run harness (`fac run`) — records the promotion as a reflection-band `09-learn.md`.
