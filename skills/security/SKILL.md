...
6. **Run the supply-chain gate (§Phase 7).** Take CI's dependency-scan JSON and SBOM and evaluate
   ...
   at or above the threshold as a hard gate; confirm a non-empty SBOM was produced. Record the
   blocking findings in the audit. A severity-less scanner (pip-audit) can't drive a severity gate —
   prefer osv-scanner/Trivy for a Python severity policy.
7. **Run the static-analysis (SAST) gate (§Phase 7).** Take CI's analyzer output (semgrep JSON or
   SARIF/CodeQL) and evaluate it via `lib/sast-report.ts` against `tech_bindings.sast`
   (`tool`, `format`, `block_severity`): normalise the findings and treat a finding at or above the
   threshold (default High) as a gate — there is no fix-available escape, the code is yours to fix.
   Record the blocking findings in the audit. `/review` surfaces these as advisory; here they gate.
8. **Audit the CI/CD pipeline (§Phase 7).** If the product has a pipeline, parse its
   ...
   registry secret, an unpinned action, or a missing required security step. Report each as a
   finding; `/pipeline` owns generating/fixing the workflow, `/security` audits it.
9. **Run the runtime & image gates when they apply (§Phase 7, Track 5).** If the product ships a
   **container image**, evaluate CI's Trivy/Grype scan via `lib/container-scan.ts` against
   ...
   at/above the risk threshold gates; a false-positive never does). Skip either scan the product
   doesn't have.
